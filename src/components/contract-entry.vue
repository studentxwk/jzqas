<template>
  <div class="contractEntry">
      ...
      <ul class="moduleItemList" :class="{ show: module.slideDown, 'hide': !module.slideDown }">
        <li class="moduleItem" v-for="item in module.items">
          ...
          <div class="formItemContainer">
            <Input v-model="item.value" v-if="item.type == 0 && item.showFlag != 0" placeholder="请输入..."
                   style="width: 300px"/>
            <Input v-model="item.value" v-if="item.type == 5" placeholder="请选择..."
                   style="width: 300px" readonly>
            <Button slot="append" icon="ios-search" @click="open(item.itemKey,item.reftype)"></Button>
            </Input>
            <Input type="number" v-model="item.value" v-if="item.type == 2" placeholder="请输入..."
                   style="width: 300px"/>
            <Checkbox v-model="item.value" v-else-if="item.type == 6"></Checkbox>
            <Date-picker v-model="item.value" type="date" placeholder="选择日期"
                         v-else-if="item.type == 3" format="yyyy-MM-dd"></Date-picker>
          </div>
        </li>
        <div class="clear">
        </div>
    ...
    <div class="contractModule">
      <div class="opt">
        <Button type="primary" @click="addRow">增行</Button>
        <Button type="warning" @click="removeRow">减行</Button>
      </div>
      <Tabs value="name1" @on-click="clickTab">
        <Tab-pane v-for="module in contractBodyModules" :key="module.id" :label="module.name" :name="module.id">
          <table class="contractBodyTable" :style="{width:module.items.length * 150 + 'px'}">
            <tr>
              <th class="tbHeader" style="width: 50px;">
                <!--<Checkbox v-model="checkedData['checkedAll']" @on-change="checkAll"></Checkbox>-->
            ...
            <tr v-for="(moduleData,index) in contractBodyModulesData[module.id]"
              ...
              <td :class="{selected:moduleData.selected}">
                <!--<Checkbox v-model="checkedData[module.id+'_'+(index+1)]"-->
    ...
    <div class="submitContainer">
      <!--<Button type="primary" long @click="submit">保存</Button>-->
      <Button type="primary" @click="submit" style="float: left;width: 80px;">保存</Button>
...
//          }
          for (var rowIndex = 0; rowIndex < rowArray.length; rowIndex++) {
            ...
            if (obj.tabCode == 'fbfyd' && this.billCode == 'H5A1') {
              var checkIsHasChildren = function (rowData, grade) {
                var isHasChildren = false;
                for (var i = 0, len = rowData.length; i < len; i++) {
                  if (rowData[i].pgrade && rowData[i].pgrade == grade) {
                    isHasChildren = true;
                  }
                }
                return isHasChildren;
              }
              //如果没有子节点则给lastGrade属性，并设置为true
              if (!checkIsHasChildren(rowArray, obj.grade)) {
                obj.lastGrade = 1;
              }
        ...
        this.contractBodyModules.map(module => {
          if (module.id == this.currentTabName) {
            ...
            if (module.tabCode == 'fbfyd' && _this.billCode == 'H5A1') {
              ...
              //树级分割线
              rowObj.leftPx = !left?10:left > 100 ? 100 : left + 10;
              if (this.contractBodyModulesData.hasOwnProperty(this.currentTabName)) {
...
//            console.log(JSON.stringify(moduleData));
            ...
            moduleData.rowData.map(row => {
              if (row.moduleId == moduleItem.id) {
                obj = {'tdData': {}, 'isChecked': false, 'tabCode': moduleItem.tabCode, 'id': row.id};
                //费用单树状表体
